...
function OrderTable({ filter, canteen }) {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const canteenMatch = canteen === 'All Canteens' || order.location === canteen;
  ...
  return (
    <>
            ...
            {noDataAvailable ? (
              ...
              filteredOrders.map((order) => (
                <tr key={order.id} className={`hover:${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  ...
                  <td className="px-6 py-4">
                    <button onClick={() => handlePreview(order)} className="text-orange-500 hover:text-orange-600">
                      <FaEye size={18} />
                    </button>
                  </td>
                </tr>
        ...
        {noDataAvailable ? (
          ...
          filteredOrders.map((order) => (
            <div key={order.id} className={`bg-white p-4 rounded-lg shadow-sm border ${darkMode ? 'bg-gray-800' : ''}`}>
              ...
              <button onClick={() => handlePreview(order)} className="mt-3 flex items-center text-sm text-orange-500">
      ...
      {/* Order Preview */}
      <FoodPreview isOpen={isPreviewOpen} onClose={() => setIsPreviewOpen(false)} order={selectedOrder} />
    </>
  );
}
