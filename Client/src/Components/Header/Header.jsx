...
const Header = ({ onCheckout }) => {
  ...
  const { data:searchedData, isLoading:searching }= useSearchingItems(searchQuery);
  ...
  const searchlist = searchedData?.length>0 ? getUniqueItemsByName(searchedData)?.slice(0, 3) : [];
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-6">
            ...
            <div className="w-full max-w-2xl relative group">
              <input
                ...
                className={`w-full px-8 py-4 rounded-full ${darkMode ? 'bg-slate-800 text-white' : 'bg-white/15 backdrop-blur-sm text-white'} transition-all duration-300`}
              />
              ...
              {searchQuery && searchQuery?.name?.length>3 && (
                <ul className={`absolute z-50 top-full left-0 w-full ${darkMode ? 'bg-slate-700 text-white' : 'bg-white/15 backdrop-blur-sm text-white'} shadow-lg rounded-lg mt-2 max-h-60 overflow-y-auto`}>
                  {searching ? (
                    ...
                    searchlist.map((item) => (
                      <li
                        key={item?._id || uuidv4()}
                        className={"p-4 hover:bg-orange-500 hover:text-white cursor-pointer transition-colors"}
            ...
            <div className="mt-12 flex gap-6">
              <button
                ...
                onClick={() => {
                  totalQty > 0 ? onCheckout() : handleLessToast()
                }}>
                Order Now
              </button>
