...
const NavigationBar = () => {
  ...
  return (
    ...
    <div
      className={`absolute ${darkMode ? 
        location.pathname==="/canteen"?"bg-slate-100/20":'bg-transparent' :
         location.pathname === "/dashboard" ? '' : 'bg-stone-800 shadow-lg'} top-0 z-10 left-0 right-0 px-4`}>
        ...
        <div className="flex items-center gap-4 md:gap-8 lg:gap-16 p-3 mr-4 md:mr-10 lg:mr-20">
          <button
           onClick={() => dispatch(toggleTheme())}
            className={`p-2 rounded-full ${darkMode ? 'bg-yellow-500' : ''} text-white hover:bg-opacity-80 transition-colors`}
          ...
          <button 
           onClick={ (state)=>{
            if(location.pathname==='/canteen')dispatch(setMerchantProfileOpen(true));
          ...
          {location.pathname==='/canteen' && <SignoutButton onSignOut={onSignOut}/>}
