...
export default function MenuManagement() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const res = editItem?._id ? await foodService.updateitem({ ...editItem, image: image || editItem.image }) : await foodService.additem(editItem, canteen?._id);
      if (res) {
        dispatch(setSuccess(`${editItem._id ? "Item updated!" : "Item Added!"}`));
        setEditItem(null);
        setIsEditing(false);
        fetchItems();
      }
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => {
            setEditItem({ name: '', price: 0, description: '', available: 'In Stock', image: '' });
            setIsEditing(true);
          }}
          className={`${darkMode ? 'bg-orange-600' : 'bg-orange-500'
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className={`rounded-lg p-6 w-full max-w-md ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <label htmlFor="imageInput" className={`w-20 h-20 mx-auto rounded-full ${darkMode ? 'bg-gray-700' : 'bg-orange-100'} flex items-center justify-center mb-3 cursor-pointer relative`}>
                {currImage ? (
              ...
              <div>
                <label className={`block text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Availability</label>
                <select
                  value={editItem.available}
                  onChange={(e) => setEditItem({ ...editItem, available: e.target.value })}
                  className={`mt-1 block w-full rounded-md border ${darkMode ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300'} px-3 py-2`}
                >
            ...
            {menuItems.length > 0 ? (
              menuItems.map((item) => (
                <tr key={item.id} className="border-b border-gray-100 last:border-0">
                  ...
                  <td className="py-4">
                    <span
                      className={`px-2 py-1 rounded-full ${item.available === 'in_stock'
              ...
              <tr>
                <td colSpan="5" className="py-4 text-center">No Items to show!</td>
              </tr>
            )}
          </tbody>
