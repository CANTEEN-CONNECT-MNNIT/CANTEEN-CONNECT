    ...
    >
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-500">({canteen?.totalRatings} reviews)</span>
        </div>
      </div>

      <div className="space-y-4" ref={reviewListRef} onScroll={handleScroll}>
        {allReviews?.length> 0 ? allReviews.map((review) => (
          <div key={review._id} className={`p-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg`}>
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">{timeFormat(review?.updatedAt)}</span>
                </div>
