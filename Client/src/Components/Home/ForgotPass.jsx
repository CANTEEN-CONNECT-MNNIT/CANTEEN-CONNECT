...
export default function ForgotPassword() {
  ...
  const dataSubmit = async (data) => {
    ...
    try {
      setIsloading(true);
      const res=token ? await authService.userResetPassword(data): await authService.userForgotPassword(data);
      if(res){
        dispatch(setSuccess(token?"Reset Password Successfully!":"Check your email please!"));
        ...
        >
          ...
          <form onSubmit={handleSubmit(dataSubmit)} className="space-y-4">
            ...
            (<div>
              ...
              />
              {errors.email && <p className="text-red-500 text-center text-sm mt-2">{errors.email.message}</p>}
            </div>)
            :
            (
              <div className="space-y-4">
                <div>
                  ...
                  {errors.password && <p className="text-red-500 text-sm text-center mt-2">{errors.password.message}</p>}
                </div>
                ...
                <div>
                  ...
                  {errors.confirmpassword && <p className="text-red-500 text-sm text-center mt-2">{errors.confirmpassword.message}</p>}
                </div>
              </div>
            )
}
                ...
                <div className="flex justify-center items-center">
                  <button
                    type="submit"
                    className={`w-full my-4  text-white ${isloading? 'bg-green-500': darkMode ? 'bg-gray-950 ' : 'bg-orange-500'} py-2 rounded-md text-md hover:bg-orange-700`}
                  >
                    {isloading?"Loading...":token? "Update Password": "Reset Password"}
                  </button>
                </div>
          </form>
        </div>
