...
import { FaWallet, FaClock, FaUtensils, FaTimes, FaCamera } from 'react-icons/fa';
import { useDispatch, useSelector } from 'react-redux';
import { setProfileOpen } from '../../Redux/Slices/pageSlice';
import userService from '../../ApiService/userApiService';
import { loginSuccess, setError, setSuccess } from '../../Redux/Slices/UserSlice';
...
const Profile = () => {
  ...
  const submitData = async(e) => {
    ...
    try {
      const res=await userService.update({name,image,email:initialState.email});
      if(res){
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 animate-fadeIn`}>
      <form 
        onSubmit={submitData} 
        className={`relative w-full max-w-md p-6 rounded-2xl shadow-xl ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} animate-slideIn`}
      >
        <button
          type="button"
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <div className="text-center text-xl">
            <input onChange={(e)=> setName(e.target.value?.trim())} value={name} className={`font-semibold bg-transparent border-b w-full text-center outline-none ${darkMode ? 'text-white' : 'text-gray-800'}`} />
            <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{initialState.email}</p>
          </div>
        </div>
        ...
        <div className={`rounded-lg p-4 mb-6 ${darkMode ? 'bg-gray-700' : 'bg-orange-50'}`}>
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xl font-bold ${darkMode ? 'text-orange-400' : 'text-orange-600'}`}>₹{initialState.balance || 0}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className={`bg-gray-50 p-3 rounded-lg text-center ${darkMode ? 'bg-gray-700' : ''}`}>
            ...
            <div className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>{initialState.activeOrders}</div>:
          ...
          <div className={`bg-gray-50 p-3 rounded-lg text-center ${darkMode ? 'bg-gray-700' : ''}`}>
            {initialState.ordersThisMonth?
              <div className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>{initialState.ordersThisMonth}</div>:
        ...
        {(edited || name!==initialState.name) && <button type="submit" className="w-full bg-orange-500 text-white py-2 rounded-lg hover:bg-orange-600 transition-colors">
          Save Changes
        </button>}
      </form>
    </div>
